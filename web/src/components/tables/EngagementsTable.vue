...
import type { ICampaignInfo } from 'src/interfaces';
import { ref } from 'vue';
...
const values = ref<ICampaignInfo[]>(props.rows);
const options = ref<ICampaignInfo[]>(props.rows);
...
const filter = () => {
  ...
  options.value = values.value.filter((item: ICampaignInfo) => {
