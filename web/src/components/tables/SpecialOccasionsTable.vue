...
import type { SpecialOccasion } from 'src/interfaces';
import { ref } from 'vue';
import { storeToRefs } from 'pinia';
import tableHeaders from './tableheaders.json'
import moment from 'moment';
const props = defineProps<{
  rows: SpecialOccasion[];
}>();
...
const values = ref<SpecialOccasion[]>(props.rows);
const options = ref<SpecialOccasion[]>(props.rows);
...
const filter = () => {
  ...
  options.value = values.value.filter((item: SpecialOccasion) => {
...
function getTimeZone(events: SpecialOccasion['events']): string {
