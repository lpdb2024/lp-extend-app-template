...
import {
  ...
  DefaultAutomaticMessagesResponseDto,
...
export class AutomaticMessagesController {
  ...
  @ApiResponse({ status: 200, description: 'List of automatic messages', type: AutomaticMessagesListResponseDto })
  async getAll(
  ...
  @Get('defaults')
  @ApiOperation({
    summary: 'Get default automatic messages',
    description: 'Gets the system default automatic messages',
  })
  @ApiParam({ name: 'accountId', description: 'LivePerson account ID' })
  @ApiResponse({ status: 200, description: 'List of default automatic messages', type: DefaultAutomaticMessagesResponseDto })
  async getDefaults(
    @Param('accountId') accountId: string,
    @Headers('authorization') authorization: string,
  ): Promise<DefaultAutomaticMessagesResponseDto> {
    const token = this.extractToken(authorization);
    const response = await this.automaticMessagesService.getDefaults(accountId, token);
  ...
  @ApiResponse({ status: 200, description: 'The automatic message', type: AutomaticMessageResponseDto })
  async getById(
    @Param('accountId') accountId: string,
  ...
  @ApiResponse({ status: 201, description: 'Automatic message created', type: AutomaticMessageResponseDto })
  async create(
    @Param('accountId') accountId: string,
    @Headers('authorization') authorization: string,
    ...
    if (Array.isArray(body)) {
      const response = await this.automaticMessagesService.createMany(accountId, token, body, revision);
      return { data: response.data, revision: response.revision };
    }

    const response = await this.automaticMessagesService.create(accountId, token, body, revision);
    return { data: response.data, revision: response.revision };
  }
  ...
  @ApiResponse({ status: 200, description: 'Automatic message updated', type: AutomaticMessageResponseDto })
  async update(
    @Param('accountId') accountId: string,
    @Param('messageId') messageId: string,
    @Headers('authorization') authorization: string,
    ...
    const response = await this.automaticMessagesService.update(accountId, messageId, token, body, revision);
    return { data: response.data, revision: response.revision };
  }
