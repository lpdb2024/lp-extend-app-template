/**
 * Actual Handle Time Service
 * Business logic for LivePerson Actual Handle Time API
 *
...
import {
  ...
  IBreakdownFilesQuery,
  IHTUBreakdownRecord,
  IBreakdownFileQuery,
} from './actual-handle-time.interfaces';

@Injectable()
export class ActualHandleTimeService extends LPBaseService {
  ...
  constructor(
    ...
    @InjectPinoLogger(ActualHandleTimeService.name)
    logger: PinoLogger,
  ) {
    super(apiService, domainService, logger);
    this.logger.setContext(ActualHandleTimeService.name);
  }

  /**
   * Get agent segments
   * Retrieves segment-level handle time data
   *
  ...
  async getAgentSegments(
    accountId: string,
    token: string,
    query: IAgentSegmentsQuery,
  ): Promise<ILPResponse<IAgentSegmentsResponse>> {
    const path = LP_API_PATHS.ACTUAL_HANDLE_TIME.AGENT_SEGMENTS(accountId);
    ...
    return this.get<IAgentSegmentsResponse>(accountId, path, token, {
      additionalParams,
    });
  }
   ...
   * Lists available HTU breakdown file resources
   *
   * @param accountId - LivePerson account ID
   * @param token - Bearer token
   * @param query - Query parameters
  ...
  async getBreakdownFilesList(
    accountId: string,
    token: string,
    query: IBreakdownFilesQuery,
  ): Promise<ILPResponse<IBreakdownFilesResponse>> {
    const path = LP_API_PATHS.ACTUAL_HANDLE_TIME.BREAKDOWN_FILES(accountId);
    ...
    return this.get<IBreakdownFilesResponse>(accountId, path, token, {
      additionalParams,
    });
  }
  ...
  /**
   * Get breakdown file
   * Downloads specific HTU breakdown file
   *
   * @param accountId - LivePerson account ID
   * @param token - Bearer token
  ...
  async getBreakdownFile(
    accountId: string,
    token: string,
    query: IBreakdownFileQuery,
  ): Promise<ILPResponse<IHTUBreakdownRecord[]>> {
    const path = LP_API_PATHS.ACTUAL_HANDLE_TIME.BREAKDOWN_FILE(accountId);
    ...
    return this.get<IHTUBreakdownRecord[]>(accountId, path, token, {
      additionalParams,
    });
  }
   ...
   */
  private buildAgentSegmentsParams(query: IAgentSegmentsQuery): Record<string, string> {
   ...
   */
  private buildBreakdownFilesParams(query: IBreakdownFilesQuery): Record<string, string> {
    const params: Record<string, string> = {
      source: query.source,
    };
  ...
  /**
   * Get handle time for a specific agent
   */
  ...
  /**
   * Get handle time for a specific skill
   */
  ...
  /**
   * Get handle time for a specific conversation
   */
